...
type PlanErrors = FieldErrors<RegistrarGastoFormData['planCuotas']>;

const PLAN_LABEL_CLASS = 'mb-2 block text-sm font-semibold text-gray-900 dark:text-white';
const planInputClass = (hasError: boolean) =>
  `w-full rounded-xl border px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-teal-600 dark:border-[#3f3f46] dark:bg-[#27272a] dark:text-white ${
    hasError ? 'border-red-500 dark:border-red-500' : 'border-gray-200'
  }`;

const PlanFieldError = ({ message }: { message?: unknown }) =>
  message ? <p className="mt-1 text-xs text-red-600">{message as string}</p> : null;
...
const PlanCuotasHeader = ({ register, isPending, isPlanEnabled }: PlanCuotasSectionProps & { isPlanEnabled: boolean }) => (
  <header className="flex flex-wrap items-center justify-between gap-3">
    ...
    <label className="relative inline-flex cursor-pointer items-center">
      <input type="checkbox" className="peer sr-only" {...register('planCuotas.habilitado')} disabled={isPending} />
...
const PlanCuotasFields = ({
  register,
  isPending,
  minDate,
  planErrors,
}: PlanCuotasSectionProps & { minDate: string; planErrors: PlanErrors }) => (
  <div className="grid gap-4 md:grid-cols-2">
    <div>
      <label className={PLAN_LABEL_CLASS} htmlFor="plan-fecha-primera-cuota">
      ...
      <input
        ...
        disabled={isPending}
        className={planInputClass(Boolean(planErrors.fechaPrimeraCuota))}
      />
      <PlanFieldError message={planErrors.fechaPrimeraCuota?.message} />
    </div>
    <div>
      <label className={PLAN_LABEL_CLASS} htmlFor="plan-cantidad-cuotas">
      ...
      <input
        ...
        disabled={isPending}
        className={planInputClass(Boolean(planErrors.cantidadCuotas))}
      />
      <p className="mt-1 text-xs text-gray-500">Máximo 36 cuotas.</p>
      <PlanFieldError message={planErrors.cantidadCuotas?.message} />
    </div>
  </div>
);

interface PlanCuotasPreviewProps {
  montos: number[];
  cantidadCuotas: number;
}

const PlanCuotasPreview = ({ montos, cantidadCuotas }: PlanCuotasPreviewProps) => {
  const regularCuota = montos[0];
  const ultimaCuota = montos[montos.length - 1];
...
const DEFAULT_PLAN = { habilitado: false, cantidadCuotas: 2 };
...
export const GastoPlanCuotasSection = ({
  ...
  const planCuotas = useWatch({ control, name: 'planCuotas' }) ?? DEFAULT_PLAN;
  ...
  const canPreview = isPlanEnabled && monto > 0 && cantidadCuotas > 0;
  const planErrors = (errors.planCuotas ?? {}) as PlanErrors;
  ...
  return (
    <section className="rounded-2xl border border-dashed border-teal-200 bg-teal-50/80 px-4 py-5 dark:border-cyan-500/40 dark:bg-cyan-500/5">
      <PlanCuotasHeader register={register} isPending={isPending} isPlanEnabled={isPlanEnabled} />

      {isPlanEnabled ? (
        <div className="mt-5 space-y-5">
          <PlanCuotasFields register={register} isPending={isPending} minDate={minDate} planErrors={planErrors} />
          <PlanCuotasPreview
            montos={canPreview ? calculatePlanCuotasMontos(monto, cantidadCuotas) : []}
